...
# APROPOS Sales
tables = ['currency_history', 'paytype', 'paytype_conv']
...
if __name__ == '__main__':
    ...
    for table in tables:
        print(table)
        # file_path = '/job_input_jsons/streams/'+env+'/kafka_to_ct/SJ1-APROPOS_SALES {0}.json'.format(table)
        file_path = '/job_input_jsons/streams/'+env+'/ct_to_current/SJ2-APROPOS_SALES {0}.json'.format(table)
        # file_path = '/job_input_jsons/streams/' + env + '/entry_point/{0}.json'.format(table)
        # file_path = '/job_input_jsons/streams/'+env+'/silver_gold_usecase/{0}.json'.format(table)
